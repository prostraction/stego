...
func MakeDCT(dctEmptyMatrix *[]float32, img *[]uint8, xPos int, yPos int, width int, channels int8, channelSelected int8) {
	...
	for x := 0; x < 8; x++ {
		for y := 0; y < 8; y++ {
			t[y*8+x] = float64(pixel(img, xPos+x, yPos+y, width, channels, channelSelected))
		}
	}
	d := dct.DCTOrthogonal(t)
	for i := 0; i < 64; i++ {
		(*dctEmptyMatrix)[i] = float32(d[i])
	}
...
func MakeIDCT(dctMatrix *[]float32, img *[]uint8, xPos int, yPos int, width int, channels int8, channelSelected int8) {
	t := make([]float64, 64)
	for i := 0; i < 64; i++ {
		t[i] = float64((*dctMatrix)[i])
	}
	d := dct.IDCTOrthogonal(t)

	for i := 0; i < 8; i++ {
		for j := 0; j < 8; j++ {
			(*img)[int(channelSelected)+(int(channels)*((j+xPos)+width*(i+yPos)))] = uint8(d[8*i+j])
		}
	}
	/*
		scale := [64]int{
